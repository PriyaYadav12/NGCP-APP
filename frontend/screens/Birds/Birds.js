...
import { logo, parrot, down, birdsBackground, birdsBackground2 } from '../../assets/asset';
...
export default function BirdScreen({ navigation }) {
  ...
  useEffect(() => {
    if(!store.ecoData.BIRD) {
      dispatchToRedux(getData({type: 'BIRD'}));
    }
  }, [dispatchToRedux]);
      ...
      <ImageBackground source={birdsBackground} style={styles.backgroundImage}>
        ...
        <View style={styles.container}>
          ...
          <View style={styles.titleBox}>
            <Text style={styles.title}>{store.ecoData.BIRD?.name[store.language]}</Text>
          </View>
          <View style={{ width: '62%' }}>
            <Text style={styles.heading}>
            {store.ecoData.BIRD?.outerHeading[store.language]}
            </Text>
          </View>
          <View style={{ width: '90%' }}>
            <Text style={styles.subHeading}>
            {store.ecoData.BIRD?.outerSubHeading[store.language]}
            </Text>
          </View>

          <DetailsScreen
            data={store.ecoData.BIRD?.ecoInfo}
            scrollViewRef={scrollViewRef}
            scrollPosition={scrollPosition}
            setContentHeight={setContentHeight}
            navigation={navigation}
            titleColor={colors.fourth.colorShade2}
            background={birdsBackground2}
...
const styles = {
  ...
  titleBox: {
    ...
    paddingVertical: 10,
    backgroundColor: colors.fourth.colorShade2,

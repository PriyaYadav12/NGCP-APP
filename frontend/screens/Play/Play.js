...
import { logo, play, down, playBackground } from '../../assets/asset';
...
export default function PlayScreen({ navigation }) {
  ...
  useEffect(() => {
    if(!store.ecoData.PLAY) {
      dispatchToRedux(getData({type: 'PLAY'}));
    }
  }, [dispatchToRedux]);
      ...
      <ImageBackground source={playBackground} style={styles.backgroundImage}>
        ...
        <View style={styles.container}>
          ...
          <View style={styles.titleBox}>
            <Text style={styles.title}>{store.ecoData.PLAY?.name[store.language]}</Text>
          </View>
          <View style={{ width: '62%' }}>
            <Text style={styles.heading}>
            {store.ecoData.PLAY?.outerHeading[store.language]}
            </Text>
          </View>
          <View style={{ width: '90%' }}>
            <Text style={styles.subHeading}>
            {store.ecoData.PLAY?.outerSubHeading?.[store.language]}
            </Text>
          </View>

          <DetailsScreen
            data={store.ecoData.PLAY?.ecoInfo}
            scrollViewRef={scrollViewRef}
            scrollPosition={scrollPosition}
            setContentHeight={setContentHeight}
            navigation={navigation}
            titleColor={colors.primary.colorShade1}
